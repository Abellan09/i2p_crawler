...
class spiderBase(scrapy.Spider):
	...
	def add_visited_links(self, link):
		...
		'''
		logger.debug("Dentro de add_visited_links(): {}".format(link))
	...
	def get_encoding(self, soup):
		...
		:param response: encoding returned by an eepsite main page / codificacion devuelta por la página principal de un eepsite
		'''
	...
	def main_page_analysis(self, response):
		...
		logger.debug("Dentro de main_page_analysis() - charset = {}".format(charset))
	...
	def get_links(self, response):
		...
		'''
		logger.info("Extracting links from {}...".format(response))
