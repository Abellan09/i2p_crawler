...
class I2P_Spider(spiderBase.spiderBase):
	...
	def __init__(self, url=None, *args, **kwargs):
		...
		self.extractor_darknet = spiderBase.LinkExtractor(tags=('a','area','base','link','audio','embed','iframe','img','input','script','source','track','video'),attrs=('href','src','srcset'),allow_domains=('i2p'),deny_extensions=())
		if url is not None:
...
class Freenet_Spider(spiderBase.spiderBase):
	...
	def __init__(self, url=None, *args, **kwargs):
		...
		self.extractor_darknet = spiderBase.LinkExtractor(tags=('a','area','base','link','audio','embed','iframe','img','input','script','source','track','video'),attrs=('href','src','srcset'),allow_domains=['127.0.0.1:8888','localhost:8888'], process_value=self.process_value)
		if url is not None:
			with open(spiderBase.darknetsettings.PATH_DATA + "languages_google.json") as f:
				self.LANGUAGES_GOOGLE = json.load(f)
			with open(spiderBase.darknetsettings.PATH_DATA + "languages_nltk.txt") as g:
